...
window.appendChatMessage = function(msgData) {
    ...
    // Debug: log for Multi-Kick
    if (Object.keys(window._mkPanels || {}).length > 0) {
        console.log('[MK-Debug] msg channel:', msgData.channel, '| panels:', Object.keys(window._mkPanels));
    }
...
function appendMkChatMessage(msgData) {
    if (!msgData || !msgData.channel) return;
    const slug = msgData.channel;
    const mkPanel = window._mkPanels[slug];
    if (!mkPanel || !mkPanel.chatEl) return;

    const sender = msgData.message?.sender || {};
    const username = sender.username || msgData.message?.username || 'unknown';
    ...
    // Build badges HTML (same as main chat)
    let badgesHtml = '';
    if (badges.length > 0) {
        badgesHtml = '<span class="kick-chat-badges">';
        badges.forEach(b => {
            if (b.type === 'broadcaster') {
                ...
                badgesHtml += `<svg class="kick-badge" viewBox="0 0 24 24" fill="var(--orange)" stroke="currentColor" stroke-width="2" width="16" height="16" style="vertical-align:middle;margin-right:2px;"><polyline points="20 12 20 22 4 22 4 12"></polyline><rect x="2" y="7" width="20" height="5"></rect><line x1="12" y1="22" x2="12" y2="7"></line><path d="M12 7H7.5a2.5 2.5 0 0 1 0-5C11 2 12 7 12 7z"></path><path d="M12 7h4.5a2.5 2.5 0 0 0 0-5C13 2 12 7 12 7z"></path></svg>`;
            } else if (b.type === 'og' || b.type === 'vip') {
                badgesHtml += `<span style="font-size: 0.65rem; background: rgba(255,215,0,0.2); color: #ffd700; padding: 1px 4px; border-radius: 3px; margin-right: 2px; vertical-align: middle; font-weight: 700;">${b.type.toUpperCase()}</span>`;
            } else {
                badgesHtml += `<span style="font-size: 0.65rem; background: rgba(255,255,255,0.1); padding: 1px 4px; border-radius: 3px; margin-right: 2px; vertical-align: middle;">${b.type}</span>`;
            }
        });
        badgesHtml += '</span>';
    }
    ...
    msgEl.innerHTML = `
        ${modActionsHtml}
        ${badgesHtml}
        <span class="kick-chat-username" style="color: ${color}; margin-right: 6px; font-weight: 700;">${username}</span>
        <span class="kick-chat-content">${parsedContent}</span>
    `;
    ...
    msgEl.addEventListener('click', async (e) => {
        ...
        try {
            await apiFetch('/api/manual-mod', {
                method: 'POST',
                body: JSON.stringify({
                    action, userId: sender.id, messageId: msgData.message.id,
                    duration, channelSlug: slug, reason: 'Multi-Kick moderasyonu',
                    username: username, messageContent: rawContent
                })
            });
